...
<head>
    ...
    <title>MenuCrest — Admin FAQs</title>
...
<body>
        ...
        <div class="pg-content" id="pgC">
            ...
            <div class="ps act">
                <div class="pg-head" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
                    <div>
                        <h1 class="pg-title">FAQs</h1>
                        <p class="pg-desc">Manage frequently asked questions</p>
                    </div>

                    <a href="<?= BASE_URL ?>/admin/faqs/create.php" class="ba text-decoration-none">
                ...
                <div class="lt">
                    ...
                    <div class="lt-r">
                        <div class="ls">
                            ...
                            <input type="text" id="customSearch" placeholder="Search FAQs...">
                        </div>

                        <div id="csvBtnContainer">
                            <!-- DataTables Buttons (CSV export) will inject here -->
                        </div>
                    </div>
                </div>
                ...
                <div class="cd">
                    <div class="cd-b p-3">
                        <div class="tw">
                            <table id="faqsTable" class="at display" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Question</th>
                                        <th>Status</th>
    ...
    <script>
        ...
        var table; // keep reference accessible to deleteFaq()
        ...
        $(function() {
            ...
            table = $('#faqsTable').DataTable({
                ...
                buttons: [{
                    ...
                    className: 'ba',
                    filename: 'faqs_export_' + new Date().toISOString().slice(0, 10)
                }]
            });
        ...
        // Called from the trash icon in each row (set via data attribute, see note below)
        function deleteFaq(id, name) {
            pendingDeleteId = id;
            $('#deleteModalText').text(
                name ? 'Delete "' + name + '"? This cannot be undone.' : 'This FAQ will be permanently deleted. This cannot be undone.'
            );
            deleteModal.show();
        }
        ...
        $('#confirmDeleteBtn').on('click', function() {
            ...
            .done(function(res) {
                deleteModal.hide();
                if (res.success) {
                    toast(res.message || 'FAQ deleted successfully', 'suc');
                    table.ajax.reload(null, false); // reload table, keep current page
                } else {
                    toast(res.message || 'Failed to delete FAQ', 'err');
                }
            })
            .fail(function() {
                deleteModal.hide();
