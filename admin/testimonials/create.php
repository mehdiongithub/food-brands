...
<head>
    ...
    <title>FoodScope — Add Testimonial</title>
    ...
    <style>
        ...
        /* --- Star rating (radio-based, no JS needed for state) --- */
        .star-rating {
            display: flex;
            flex-direction: row-reverse; /* reversed order lets CSS ~ sibling selector light up correctly */
        ...
        /* Keep selected stars lit after choosing */
...
<body>
    ...
    <div id="main">
        <div class="pg-content" id="pgC">
            ...
            <div class="pg-head" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
                <div>
                    <h1 class="pg-title">Add Testimonial</h1>
                    <p class="pg-desc">Add a new customer testimonial</p>
                </div>
                    ...
                    <form id="addTestimonialForm" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <small style="color:var(--muted);font-size:.72rem;">Optional. JPG, PNG or WEBP. Max 2MB.</small>
    ...
    <script>
        ...
        $('#addTestimonialForm').on('submit', function (e) {
            ...
            // --- Rating is a native radio group, so we just check if one is checked ---
            ...
            // Make sure the latest Quill content is synced (covers case where
            // user never triggered a text-change event, e.g. left it empty)
            $('#review').val(quill.root.innerHTML);

            // Treat Quill's "empty" placeholder markup as truly empty
            ...
            $.ajax({
                url: '../../api/testimonials/add-testimonial.php',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
            ...
            .done(function (res) {
                if (res.success) {
                    toast(res.message || 'Testimonial created successfully', 'suc');
                    setTimeout(function () {
            ...
            .always(function () {
                $btn.prop('disabled', false).html('<i class="fas fa-save"></i> Create Testimonial');
            });
        });
    </script>
</body>
