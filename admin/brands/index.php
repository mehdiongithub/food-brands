...
<head>
    ...
    <title>MenuCrest — Admin Brands</title>
    ...
    <style>
        /* Toggle button — plus/minus, rotates smoothly like a mobile nav hamburger */
        .row-toggle-btn {
            width: 30px;
            height: 30px;
            border-radius: 50%;
        ...
        tr.row-expanded .row-toggle-btn {
            background: var(--accent, #E85D04);
            border-color: var(--accent, #E85D04);
            color: #fff;
        }
        tr.row-expanded .row-toggle-btn i {
            transform: rotate(135deg); /* plus rotates into an X/minus look */
        }

        /* Child row — full width, animated slide */
        td.details-control-cell {
            padding: 0 !important;
        ...
        .child-row-inner.open {
            max-height: 500px; /* generous cap; content is short so this is fine */
        }
    </style>
</head>
            ...
            <div class="ps act">
                <div class="pg-head" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
                    <div>
                        ...
                        <p class="pg-desc">Manage food brand partnerships</p>
                    </div>

                    <?php if (currentUserRole() === 'admin'): ?>
                    <a href="<?= BASE_URL ?>/admin/brands/create.php" class="ba text-decoration-none">
                ...
                <div class="lt">
                    ...
                    <div class="lt-r">
                        <div class="ls">
                            ...
                            <input type="text" id="customSearch" placeholder="Search brand name...">
                        </div>
                ...
                <div class="cd">
                    <div class="cd-b p-3">
                        <div class="tw">
                            <table id="brandsTable" class="at display" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Brand</th>
                                        <th>Founded</th>
...
<script>
    ...
    $(function() {
        ...
        table = $('#brandsTable').DataTable({
            ...
            columns: [
                ...
                { data: "founded_year", orderable: true },
            ...
            drawCallback: function () {
                $('#brandsTable tbody tr.row-expanded').removeClass('row-expanded');
            }
        });
        ...
        // IMPORTANT: bound via table.on(), not $(selector).on() — this uses DataTables'
        // own delegated event system tied to the table instance itself, which is
        // guaranteed to exist immediately and survives every redraw/pagination/sort.
    ...
    function deleteBrand(id, name) {
        ...
        $('#deleteModalText').text(
            name ? 'Delete "' + name + '"? This cannot be undone.' : 'This brand will be permanently deleted. This cannot be undone.'
        );
        deleteModal.show();
    }
    ...
    $('#confirmDeleteBtn').on('click', function() {
        ...
        $.ajax({
            url: '../../api/brands/delete-brand.php',
            type: 'POST',
        ...
        .done(function(res) {
            deleteModal.hide();
            if (res.success) {
                toast(res.message || 'Brand deleted successfully', 'suc');
                table.ajax.reload(null, false);
            } else {
                toast(res.message || 'Failed to delete brand', 'err');
            }
        })
        .fail(function() {
            deleteModal.hide();
