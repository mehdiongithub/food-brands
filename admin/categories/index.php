...
<head>
    ...
    <title>FoodScope — Admin Categories</title>
        ...
        <div class="pg-content" id="pgC">
            ...
            <div class="ps act">
                <div class="pg-head" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
                    <div>
                        <h1 class="pg-title">Categories</h1>
                        <p class="pg-desc">Manage category information</p>
                    </div>
                    <?php
                    if ($isAdmin = (currentUserRole() === 'admin')){
                    ?>
                    <a href="<?= BASE_URL ?>/admin/categories/create.php" class="ba text-decoration-none">
                ...
                <div class="lt">
                    ...
                    <div class="lt-r">
                        <div class="ls">
                            ...
                            <input type="text" id="customSearch" placeholder="Search categories...">
                        </div>
                ...
                <div class="cd">
                    <div class="cd-b p-3">
                        <div class="tw">
                            <table id="categoriesTable" class="at display" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <th>Status</th>
    ...
    <script>
        ...
        $(function() {
            table = $('#categoriesTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "../../api/categories/categories.php",
                    type: "POST",
                    data: function(d) {
        ...
        function deleteCategory(id, name) {
            ...
            $('#deleteModalText').text(
                name ? 'Delete "' + name + '"? This cannot be undone.' : 'This Category will be permanently deleted. This cannot be undone.'
            );
            deleteModal.show();
        }
        ...
        $('#confirmDeleteBtn').on('click', function() {
            ...
            $.ajax({
                url: '../../api/categories/delete-category.php',
                type: 'POST',
            ...
            .done(function(res) {
                deleteModal.hide();
                if (res.success) {
                    toast(res.message || 'Category deleted successfully', 'suc');
                    table.ajax.reload(null, false);
                } else {
                    toast(res.message || 'Failed to delete category', 'err');
                }
            })
            .fail(function() {
                deleteModal.hide();
