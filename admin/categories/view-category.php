...
<head>
    ...
    <title>FoodScope — View Category</title>
...
<body>
    ...
    <div id="main">
        <div class="pg-content" id="pgC">
            ...
            <div class="pg-head" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:12px;">
                <div>
                    <h1 class="pg-title">View Category</h1>
                    <p class="pg-desc">Category details</p>
                </div>
                <div style="display:flex;gap:10px;">
                    <?php if (currentUserRole() === 'admin'): ?>
                    <a href="edit-category.php?token=<?= urlencode(encryptId($id)) ?>" class="ba text-decoration-none">
        ...
        .done(function (res) {
            if (res.success) {
                ...
                $('#viewContainer').html('<div style="text-align:center;padding:40px;color:var(--muted);">' +
                    ...
                    '<p style="margin-top:10px;">' + escapeHtml(res.message || 'Category not found') + '</p></div>');
            }
        })
        .fail(function () {
            toast('Failed to load category details', 'err');
            $('#viewContainer').html('<div style="text-align:center;padding:40px;color:var(--muted);">' +
                '<p>Something went wrong while loading this category.</p></div>');
        });
    });
    ...
    function renderCategory(c) {
        var imageHtml;
        if (c.image) {
            imageHtml = "<img src='" + BASE_URL_JS + "/" + c.image + "' alt='" + escapeHtml(c.name) + "' style='width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid var(--border);'>";
        } else {
        ...
        // description is trusted, already-sanitized HTML stored by add-category.php —
        // rendered directly so formatting (bold, lists, etc.) displays correctly
        var descriptionHtml = c.description || '';
